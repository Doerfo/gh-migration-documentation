...
This page covers the configuration of **Microsoft Entra ID** (formerly Azure AD) as the identity provider for **GitHub Enterprise Cloud** with **Enterprise Managed Users (EMU)**.

The integration consists of two parts:
1. **OIDC SSO** – Authentication via OpenID Connect
2. **SCIM Provisioning** – Automatic user lifecycle management
...
{: .warning }
> Each Entra ID tenant supports only **one** OIDC integration with Enterprise Managed Users. If multiple enterprises need to connect to the same tenant, use SAML for the additional enterprises.
...
| # | Prerequisite | Details |
|---|---|---|
| 1 | OIDC SSO enabled | Completed in Part 1 |
...
{: .note }
> Entra ID does **not** support provisioning nested groups. Ensure all user assignments are in flat groups.
...
| Entra ID Role Value | GitHub Enterprise Role |
|---|---|
| `Enterprise Owner` | Full administrative access |
| `Enterprise Member` | Default member access |
| `Guest Collaborator` | Limited access to internal repositories only |

- Map Entra ID groups to GitHub **teams** for automatic organisation membership and repository access.

> **Reference:** [Managing team memberships with identity provider groups](https://docs.github.com/en/enterprise-cloud@latest/admin/identity-and-access-management/using-enterprise-managed-users-for-iam/managing-team-memberships-with-identity-provider-groups)
...
> **Reference:** [Username considerations for external authentication](https://docs.github.com/en/enterprise-cloud@latest/admin/identity-and-access-management/managing-iam-for-your-enterprise/username-considerations-for-external-authentication)
